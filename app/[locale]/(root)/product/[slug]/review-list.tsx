...
// import { useToast } from '@/hooks/use-toast'  <-- supprimer cet import
import { toast } from 'sonner'  // <-- import toast depuis sonner
import {
  createUpdateReview,
...
export default function ReviewList({
  ...
  const reload = async () => {
    try {
      ...
      toast.error(t('Error in fetching reviews'))  // <-- toast error ici
    }
  }
  ...
  // const { toast } = useToast()  <-- supprimer cette ligne
  ...
  const onSubmit: SubmitHandler<CustomerReview> = async (values) => {
    ...
    if (!res.success) {
      toast.error(res.message)  // <-- toast error ici
      return
    }
    setOpen(false)
    reload()
    toast.success(res.message)  // <-- toast success ici
  }
